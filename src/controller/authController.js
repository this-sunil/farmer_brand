...
const createUserTable=async()=>{
    ...
    const {rows}=await pool.query(existUser);
    if(rows.length===0){
        admin();
    }
};
...
export const registerController=async(req,res)=>{
    ...
    try {
        const query=`INSERT INTO users(name,phone,photo,pass,state,city) VALUES($1,$2,$3,$4,$5,$6) RETURNING *`;
        const hashPass=await bcrypt.hash(pass,10);
        const {rows}=await pool.query(query,[name,phone,"",hashPass,state,city]);
        delete rows[0].pass;
        const token=await generateToken(rows[0].role);
        if(rows.length>0){
            return res.status(200).json({
